    ...
    public static void main(final String[] args) {
        ...
        for (int i = 0; i < fileArray.length; i++) {
            ...
            if (fileArray[i].isFile() && fileArray[i].getName().startsWith("dictionary")) {
                ...
                BoggleBoard boggleBoardObj = null;

                for (int j = 0; j < fileArray.length; j++) {

                    if (fileArray[j].isFile() && fileArray[j].getName().startsWith("board-points")) {
        
                        boggleBoardObj = new BoggleBoard(path + fileArray[j].getName());

                        System.out.println("Input File : " + fileArray[i].getName() + ", Input Board : " + fileArray[j].getName());
                        System.out.println("");    
